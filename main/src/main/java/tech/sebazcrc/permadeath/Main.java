...
public final class Main extends JavaPlugin implements Listener {
    ...
    private final Map<Integer, Boolean> registeredDays = new HashMap<>();
    @Getter
    private final ArrayList<Player> doneEffectPlayers = new ArrayList<>();
    ...
    private void tickAll() {
        Bukkit.getScheduler().runTaskTimer(this, () -> {
            ...
            if (!loaded) {
                ...
                if (getConfig().getBoolean("Toggles.Replace-Mobs-On-Chunk-Load") && spawnListener != null) {
            ...
            if (!Bukkit.getOnlinePlayers().isEmpty() && SPEED_RUN_MODE) {
                playTime++;
            ...
            if (getDay() >= 60 && !getConfig().getBoolean("DontTouch.Event.LifeOrbEnded") && getOrbEvent() != null && !getOrbEvent().isRunning()) {
        ...
        for (Player player : Bukkit.getOnlinePlayers()) {
            ...
            if (getDay() >= 50) {
                ...
                if (player.getWorld().getEnvironment() == World.Environment.NETHER && getDay() < 60) {
                    int rand = this.random.nextInt(4500) + 1;

                    if (rand <= 10 && player.getWorld().getLivingEntities().size() < 110) {
    ...
    private void tickEvents() {
        if (this.orbEvent != null && this.orbEvent.isRunning()) {
            if (this.orbEvent.getTimeLeft() > 0) {
                ...
                Bukkit.broadcastMessage(TextUtils.format(prefix + "&cSe ha acabado el tiempo para obtener el Life Orb, ¡sufrid! ahora tendreís 8 contenedores de vida menos."));
        ...
        if (this.shulkerEvent != null && this.shulkerEvent.isRunning()) {
            if (this.shulkerEvent.getTimeLeft() > 0) {
                ...
                Bukkit.broadcastMessage(TextUtils.format(prefix + "&eEl evento de &c&lX2 Shulker Shells &eha acabado."));
    ...
    private void registerListeners() {
        String logPrefix = "&e[PermaDeath] &7> ";

        if (Boolean.FALSE.equals(registeredDays.get(1))) {
            registeredDays.replace(1, true);
            this.getServer().getPluginManager().registerEvents(new AnvilListener(this), this);
        }

        if (DateManager.getInstance().getDay() >= 20 && Boolean.FALSE.equals(registeredDays.get(20))) {
            ...
            Bukkit.getConsoleSender().sendMessage(TextUtils.format(logPrefix + "&eSe han registrado los cambios de Mobs pacíficos hostiles."));
        }

        if (DateManager.getInstance().getDay() >= 30 && endManager == null && endData == null && Boolean.FALSE.equals(registeredDays.get(30))) {
            ...
            if (runningPaperSpigot) {
                ...
                Bukkit.getConsoleSender().sendMessage(TextUtils.format(logPrefix + "&eSe han registrado cambios especiales para &c&lPaperMC&e."));
            }
        }

        if (DateManager.getInstance().getDay() >= 40 && Boolean.FALSE.equals(registeredDays.get(40))) {
            ...
            Bukkit.getConsoleSender().sendMessage(TextUtils.format(logPrefix + "&eSe han registrado cambios para el día &b40"));

            if (Bukkit.getPluginManager().getPlugin("WorldEdit") == null) {
                Bukkit.broadcastMessage(TextUtils.format(logPrefix + "&4&lNo se pudo registrar TheBeginning ya que no se ha encontrado el plugin &7WorldEdit"));
                Bukkit.broadcastMessage(TextUtils.format(logPrefix + "&7Si necesitas soporte entra a este discord: &e" + Utils.SPIGOT_LINK));
                return;
            }
            this.beData = new BeginningDataManager(this);
            this.begginingManager = new BeginningManager(this);
            Bukkit.getConsoleSender().sendMessage(TextUtils.format(logPrefix + "&eSe han registrado cambios de TheBeginning"));
        }

        if (DateManager.getInstance().getDay() >= 50 && Boolean.FALSE.equals(registeredDays.get(50))) {
            if (this.recipes == null) {
                this.recipes = new RecipeManager(this);
                this.recipes.registerRecipes();
            }
            Bukkit.getConsoleSender().sendMessage(TextUtils.format(logPrefix + "&eSe han registrado cambios para el día &b50"));
            this.recipes.registerD50Recipes();
            registeredDays.replace(50, true);
        }

        if (DateManager.getInstance().getDay() >= 60 && Boolean.FALSE.equals(registeredDays.get(60))) {
            if (this.recipes == null) {
                this.recipes = new RecipeManager(this);
                this.recipes.registerRecipes();
                this.recipes.registerD50Recipes();
            ...
            Bukkit.getConsoleSender().sendMessage(TextUtils.format(logPrefix + "&eSe han registrado cambios para el día &b60"));
    ...
    public void generateOfflinePlayerData() {
        for (OfflinePlayer off : Bukkit.getOfflinePlayers()) {
            if (off == null || off.getName() == null) continue;
...
-Messages.coords-msg-enable", true);
        c.set("TotemFail.Enable", true);
        c.set("TotemFail.Medalla", "&7¡El jugador %player% ha usado su medalla de superviviente!");
        c.set("TotemFail.ChatMessage", "&7¡El tótem de &c%player% &7ha fallado!");
        c.set("TotemFail.ChatMessageTotems", "&7¡Los tótems de &c%player% &7han fallado!");
