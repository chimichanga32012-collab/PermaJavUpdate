...
public class Cuboid implements Cloneable, ConfigurationSerializable, Iterable<Block> {
    ...
    public List<Block> getBlocks() {
        ...
        if (world != null) {
            // Se añaden límites dinámicos compatibles con alturas modificadas e inferiores a 0 (1.18+)
            int minX = this.minimumPoint.getBlockX();
            int maxX = this.maximumPoint.getBlockX();
            int minY = Math.max(this.minimumPoint.getBlockY(), world.getMinHeight());
            int maxY = Math.min(this.maximumPoint.getBlockY(), world.getMaxHeight() - 1);
            int minZ = this.minimumPoint.getBlockZ();
            int maxZ = this.maximumPoint.getBlockZ();
