...
public class EntityTeleport {
    ...
    private boolean eq() {
        ...
        // Validamos que el cálculo no intente buscar bloques por encima del límite del mapa
        int startY = Math.min((int) y, world.getMaxHeight() - 1);
        Block b = this.world.getBlockAt((int) x, startY, (int) z);
        int minHeight = this.world.getMinHeight();

        // Cambiado b.getY() > 0 por minHeight para dar soporte nativo a capas negativas (1.18+)
        while (b.getY() > minHeight && b.getType().isAir()) {
            b = b.getRelative(BlockFace.DOWN);
        }

        if (b.getY() <= minHeight) return false;
