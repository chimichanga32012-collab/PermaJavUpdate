...
public class GatoGalacticoTask extends BukkitRunnable {
    ...
    public void run() {
        if (time > 0) {
            Bukkit.broadcast(TextUtils.format("&eUn gato galáctico invocará un mob al azar en: &b" + time));
        ...
        if (time == 0) {
            ...
            if (type == EntityType.CAT) {
                ...
                cat.getWorld().spawnEntity(cat, EntityType.PUFFERFISH);
                ...
                EntityEquipment jessEq = jess.getEquipment();
                if (jessEq != null) {
                    jessEq.setItemInMainHand(new ItemStack(Material.GOLDEN_APPLE, 2));
                    jessEq.setItemInMainHandDropChance(0);
                }

                EntityEquipment carlosEq = carlos.getEquipment();
                if (carlosEq != null) {
                    carlosEq.setItemInMainHand(new ItemStack(Material.GOLD_INGOT, 32));
                    carlosEq.setItemInMainHandDropChance(0);
                }
                ...
                int i = (Main.instance.getDay() < 60) ? random.nextInt(3) + 1 : random.nextInt(2) + 1;
                ...
                ItemStack bannerItem = new ItemStack(Material.BLACK_BANNER, 1);
                ...
                if (m != null) {
                    ...
                    bannerItem.setItemMeta(m);
                }

                if (eq != null) {
                    eq.setHelmet(bannerItem);
                    ...
                    eq.setItemInMainHand(new ItemBuilder(Material.BOW)
                            .addEnchant(Enchantment.PUNCH, 5)
                            .addEnchant(Enchantment.POWER, 100)
                            .build());
                    eq.setItemInMainHandDropChance(0);
                }
            }

            Bukkit.broadcast(TextUtils.format("&eUn gato galáctico ha invicado un(a) &c&l" + entidad + " &7(" + cat.getBlockX() + ", " + cat.getBlockY() + ", " + cat.getBlockZ() + ")"));
            this.cancel();
        }
    }
}
